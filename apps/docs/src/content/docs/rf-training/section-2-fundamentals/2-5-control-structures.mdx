...
### Run all three control structures in one suite

<RunnableRobot
  client:visible
  fileName="control_structures.robot"
  caption="IF/ELSE, FOR (both IN and IN RANGE), and TRY/EXCEPT/FINALLY — one suite, three tests."
  initialCode={`*** Test Cases ***
Conditional Based On Weight
    VAR    \${weight: int}    8200
    IF    \${weight} > 25000
        Log    Overweight — split the shipment
    ELSE
        Log    OK to load (\${weight} kg)
    END

Iterate Over Containers And Docks
    @{containers}    Create List    MRKU1234567    TCLU7654321    MSKU9988776
    FOR    \${container}    IN    @{containers}
        Log    Inspecting \${container}
    END
    FOR    \${dock}    IN RANGE    1    4
        Log    Dock \${dock} ready
    END

Try Except Continues After Failure
    TRY
        Fail    simulated failure
    EXCEPT
        Log    Caught the failure — test keeps going
    FINALLY
        Log    Cleanup step always runs
    END
`}
/>
